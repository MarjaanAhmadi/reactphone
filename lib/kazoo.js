...
(function () {
	...
	kazoo.register = function (params) {
		...
		if (kazoo.version === 'rtmp') {
					...
					case 'src': {
						if (event.newValue !== null) { // If src newValue has been set
							...
							if (kazoo.rtmp.isConnecting) {
								...
								if (params.reconnectMaxAttempts) {
			...
			kazoo.jssip.userAgent.on('newRTCSession', function (arg) {
				...
				console.log("kazoo.jssip.session", kazoo.jssip.session);
				console.info('invite', session);
			...
			kazoo.jssip.sessionEventHandlers = {
				...
				'hold': function (arg) {
					console.info('hold', arg)
				},
				'unhold': function (arg) {
					console.info('unhold', arg)
				},
				'isonHold': function (arg) {
					console.log('isonHold', arg);
				},
				'renegotiate': function (arg) {
					console.info('renegotiate', arg);
				},
				'ended': function (arg) {
					console.info('bye', arg);
					delete kazoo.jssip.session;
	...
	kazoo.transfer = function (dest) {
		if (kazoo.version === 'rtmp') {
			kazoo.rtmp.phone.callProperty('call', 'transfer', dest);
		} else if (kazoo.version === 'sipjs') {
			kazoo.sipjs.session.refer(dest);
		} else if (kazoo.version === 'jssip') {
			kazoo.jssip.session.refer(dest);
		}

		kazoo.params.onTransfer && kazoo.params.onTransfer();
	};

	kazoo.AttendedTransfer = function (target) {
		if (kazoo.version === 'rtmp') {
			...
			console.log("session--------------------------------", JsSIP)
			// kazoo.connect(target);

			debugger
			console.log("-------------------------", kazoo.jssip.session)
			let options = {
				replaces: sessions[1],
				eventHandlers: kazoo.jssip.bindSessionEvents
			};
			debugger
			// sessions[0].refer(sessions[1]._remote_identity._uri, options);
			debugger;
			console.log(kazoo.jssip.session);
			sessions[0].refer(target, options);

			kazoo.params.onTransfer && kazoo.params.onTransfer();


			sessions[1].on('peerconnection', function (data) {
				// debugger
				//
				// //Get the remote stream from session with A
				// var remoteStream = sessions[0]._connection.getRemoteStreams()[0];
				//
				// //Remove remote stream from A
				// sessions[0]._connection.removeTrack(remoteStream);
				//
				// //Add remote stream from A to session with C, not sure about this....
				// sessions[1]._connection.addTrack(remoteStream);
	...
	kazoo.hangupAttended = function () {
		sessions[1]._remote_identity._uri._user.connect(sessions[0]._remote_identity._uri._user);
		(sessions[0], sessions[1]).hangup();
	}
	...
	kazoo.connect = function (destination) {
		...
		if (kazoo.version === 'rtmp') {
			...
			sessions.push(kazoo.jssip.session);
			debugger

		}
	};
	kazoo.startConference = function (remoteAudio) {
		kazoo.conference(sessions, remoteAudio);
	}
...
//remoteAudioId => the ID of your <audio> element to play the received streams
	kazoo.conference = function (sessions, remoteAudio) {
		//take all received tracks from the sessions you want to merge
		var receivedTracks = [];
		sessions.forEach(function (session) {
			debugger
			if (session !== null && session !== undefined) {
				session._connection.getReceivers().forEach(function (receiver) {
		...
		sessions.forEach(function (session) {
			if (session !== null && session !== undefined) {
				...
				session._connection.getSenders().forEach(function (sender) {
					var sourceStream = context.createMediaStreamSource(new MediaStream([sender.track]));
					sourceStream.connect(mixedOutput);
				});
				session._connection.getSenders()[0].replaceTrack(mixedOutput.stream.getTracks()[0]);
			}
		});
	...
	kazoo.holdSession = function (hold, success, error) {
		if (kazoo.version === 'rtmp') {
			...
			if (kazoo.jssip.hasOwnProperty('session')) {
				...
				if (hold) {
					...
					const test = kazoo.jssip.session.hold(options, () => {
					});
				} else {

					kazoo.jssip.session.unhold(options, () => {
					});
				}
